...
class DataLogger(threading.Thread):
    ...
    def run(self):
        with self.file.open("w", newline="") as f:
            writer = csv.writer(f)
            writer.writerow(
                ["ts_wall"] + list(MeasurementFrame.__dataclass_fields__.keys()) + ["throttle_pct"]
            )
            while not self.stop_evt.is_set():
                try:
                    frame, throttle_pct = self.q.get(timeout=0.5)
                    writer.writerow([time.time(), *frame.to_tuple(), throttle_pct])
                except queue.Empty:
                    continue
